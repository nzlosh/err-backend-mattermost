...
class MattermostBackend(ErrBot):
    ...
    def _status_change_event_handler(self, message):
        ...
        if status == 'online':
            ...
            log.error(
                "It appears the Mattermost API changed, "
                "I received an unknown status type %s" % status
            )
            status = ONLINE
    ...
    def build_identifier(self, txtrep):
        """
        Convert a textual representation into a :class:`~MattermostPerson`
            or :class:`~MattermostRoom`
    ...
    def send_card(self, card: Card):
        ...
        try:
            ...
            log.exception(
                ...
                " to send a card to %s.[%s]" % (to_humanreadable, card)
            )
    ...
    def channels(self, joined_only=False):
        channels = []
        channels.extend(
            self.driver.channels.get_channels_for_user(user_id=self.userid, team_id=self.teamid)
        )
        if not joined_only:
